...
f_score=[]
i=-1
#print "metadata model prediction"
for x in test_files:
    print x
for x in users:
    i+=1
    ...
    metamodel = clf.predict(x_validation)
    metadata_feature.append(metamodel)
    print x +" "+str(clf.predict(x_test))
...
metadata_feature_df = pd.DataFrame(data=metadata_feature,columns=users)
final_df.update(metadata_feature_df)
...
validation=pd.merge(validation,metadata_feature_df,left_index=True, right_index=True,how='inner')
    ...
    for x in predicted_users:
        features.append(x)
print "Collaboration model prediction"
for x in users:
    features.append(x)
result = []
for x in users:
    ...
    cfmodel = clf.predict(x_test)
    print x +" "+str(cfmodel)
    
    

average_precision = sum(precision)/i
average_recall = sum(recall)/i
